...
export function AppShell() {
  ...
  const buddyTarget = location.pathname.startsWith("/buddy") ? (params.get("context") || "/werkbank") : location.pathname;
  return <div className={`application-frame ${collapsed ? "application-frame--collapsed" : ""}`}>
    ...
    <Drawer open={drawer === "library"} title="Bibliothek" onClose={closeDrawer}><div className="library-quick-access"><p>Öffne die vollständige Bibliothek, um Materialien zu suchen, zu filtern und als Favoriten zu speichern.</p><Link className="button button--primary" to="/bibliothek" onClick={closeDrawer}>Bibliothek öffnen</Link><Link className="button button--secondary" to="/materialien/neu" onClick={closeDrawer}>Neues Material erstellen</Link></div></Drawer>
  </div>;
}
