...
export function EmptyState({ title = "Noch kein Material", description = "Hier erscheinen später vorbereitete Materialien. Dieser Prototyp legt keine Fachdaten an.", children, action }: { title?: string; description?: string; children?: ReactNode; action?: ReactNode }) { return <div className="state-card"><FileQuestion aria-hidden="true" /><h3>{title}</h3><p>{description}</p>{action ?? children ?? <Button variant="secondary">Beispiel ansehen</Button>}</div>; }
...
export function ErrorState({title="Ansicht konnte nicht geladen werden",children}: {title?:string;children?:ReactNode}={}) { return <div className="state-card state-card--error" role="alert"><AlertTriangle aria-hidden="true" /><h3>{title}</h3><p>{children??"Die lokalen Daten bleiben erhalten. Versuche es erneut."}</p></div>; }
