...
import { addMaterial, readTopics, updateTopic, type TopicLesson, type TopicRecord } from "./topicStore";
import "./topic-center.css";

type LessonTextField = Exclude<keyof TopicLesson, "id" | "materialIds">;
...
export function TopicLessonPage() {
  ...
  const saveLesson = (field: LessonTextField, value: string) => {
    const lessons = topic.lessons.map((entry, index) => index === lessonIndex ? { ...entry, [field]: value } : entry);
    const next: TopicRecord = { ...topic, lessons };
